...
**Teaches:** session auth gate on `/boson` and the in-memory dashboard KPI shape `boson-backend` builds for the UI.
...
**Next step:** Mount `<BosonRoutes />` in a product host with Higgs + Boson coordinator.
